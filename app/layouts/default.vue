...
<template>
  ...
  >
    ...
    <template v-if="tocData?.hasToc && !tocData?.isEditorEnabled" #right>
      <UContentToc :title="tocData.tocTitle" :links="tocData.tocLinks">
        <template v-if="tocData.tocBottom" #bottom>
          <div
            class="hidden lg:block space-y-6"
            :class="{ '!mt-6': tocData.tocLinks?.length }"
          >
            <USeparator v-if="tocData.tocLinks?.length" type="dashed" />
